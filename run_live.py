...
def stitch_chapters(chapter_scripts, episode_number: int, episode_title: str,
                    hook_line: str, cliffhanger: str) -> EpisodeScript:
    ...
    for ch in chapter_scripts:
        ...
        for vp in ch.visual_prompts:
            ...
            all_panels.append(VisualPrompt(
...
async def run_live(plot: str, series_id: str, num_episodes: int = 1, start_episode: int = 1):
    logger.info("=" * 60)
    ...
    for ep_plan in plan.episodes:
        ...
        if script_path.exists():
            ...
            for ch in ch_plan.chapters:
                ch_script_path = story_dir / f"episode_{ep_plan.episode_number:02d}_chapter_{ch.chapter_number:02d}.json"

                if ch_script_path.exists():
                    logger.info("   Loading existing Chapter %d from %s", ch.chapter_number, ch_script_path)
        ...
        try:
            ...
            if final_video.exists():
                logger.info("✅ Episode %d complete: %s", ep_plan.episode_number, final_video.resolve())
            else:
                logger.error("❌ Episode %d: Video not found after pipeline", ep_plan.episode_number)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The story plot (e.g., 'His wife dumped him because he's poor. She doesn't know he's a billionaire.')",
    )
    parser.add_argument("--series", "-s", type=str, default="revenge_series", help="Series ID")
    parser.add_argument("--episodes", "-e", type=int, default=1, help="Number of episodes to generate (default: 1)")
    parser.add_argument("--start", type=int, default=1, help="Start from episode N (skips earlier ones)")

    args = parser.parse_args()

    settings.enable_whisper = False

    asyncio.run(run_live(args.plot, args.series, args.episodes, args.start))
